...
BASE_DIR="scripts/performance_exp/results/r${SERVICE_REPLICAS}"
mkdir -p "$BASE_DIR"
...
for p in $PARALLEL_LIST; do
  ...
  echo "==> $label"
  ./scripts/performance_exp/bench.sh "$p" "$label" "$BASE_DIR"
done

echo
echo "Done. Results in $BASE_DIR"
echo "Now running:"
echo "./scripts/performance_exp/summarize_plot.sh \"$BASE_DIR\""

./scripts/performance_exp/summarize_plot.sh $BASE_DIR