...
class Settings:
    ...
    LITE_LLM_ADDRESS: str = os.getenv(
        "FREVAGPT_LITE_LLM_ADDRESS", "http://litellm:4000"
    )
    ...
    MONGODB_COLLECTION_NAME_EMB: str = os.getenv(
        "FREVAGPT_MONGODB_COLLECTION_NAME_EMB", "embeddings"
    )
    CLEAR_MONGODB_EMBEDDINGS: bool = os.getenv(
        "FREVAGPT_CLEAR_MONGODB_EMBEDDINGS", ""
    ).lower() in {"1", "true", "yes"}
    MCP_REQUEST_TIMEOUT_SEC: int = int(
        os.getenv("FREVAGPT_MCP_REQUEST_TIMEOUT_SEC", "600")
    )
