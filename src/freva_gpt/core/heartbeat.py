...
from freva_gpt.services.streaming.stream_variants import SVServerHint
...
async def heartbeat_content():
    ...
    heartbeat["cpu_last_minute"] = psutil.getloadavg()[
        0
    ...
    while found_some:
        ...
        for p in psutil.process_iter(["pid", "ppid"]):
            if (
                p.info["ppid"] in process_list
                and p.info["pid"] not in process_list
            ):
