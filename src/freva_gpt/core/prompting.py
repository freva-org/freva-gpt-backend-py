...
def _resolve_baseline_dir() -> Path:
    for d in BASELINE_DIRS:
        if all(
            (d / name).is_file()
            for name in (STARTING_TXT, SUMMARY_TXT, EXAMPLES_JL)
        ):
            return d
...
def get_entire_prompt(
    ...
    if model_is_gpt_5(model):
        logger.info(
            "GPT-5 placeholder active: baseline prompts used for model='%s'.",
            model,
        )
...
def get_entire_prompt_json(user_id: str, thread_id: str, model: str) -> str:
    ...
    return json.dumps(
        get_entire_prompt(user_id, thread_id, model), ensure_ascii=False
    )
