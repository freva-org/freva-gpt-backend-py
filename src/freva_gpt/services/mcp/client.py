...
class McpClient:
    ...
    def __init__(
        ...
        self._http = httpx.Client(
            base_url=self.base_url, timeout=httpx.Timeout(300.0)
        )
    ...
    def _start_session(self) -> str:
        ...
        notify_resp = self._http.post(
            "/mcp", headers=self._headers(), json=notify_body
        )
        ...
        if isinstance(notify_payload, dict) and notify_payload.get("error"):
            raise McpError(
                f"MCP notifications/initialized failed: {notify_payload['error']}"
            )
