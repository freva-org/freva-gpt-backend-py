...
def compute_hash(doc):
    ...
    if type(doc.page_content) == list or str:
        ...
        raise TypeError(
            "Unknown content type in document. The content should either be string or a list."
        )
...
def add_vector_search_index_to_db(
    ...
    if "vector_index" not in index_names:
        ...
        while (
            collection.list_search_indexes().to_list()[0]["status"] != "READY"
        ):
            time.sleep(1)
...
def get_new_or_changes_documents(documents, db):
    ...
    for doc in documents:
        ...
        if is_doc_in_db(doc, db):
            logger.info(
                f"Embeddings already exist for {doc.metadata.get('source')}-{doc.metadata.get('chunk_id')}"
            )
            continue  # Skip re-embedding
        else:
            logger.info(
                f"Creating embeddings for {doc.metadata.get('source')}-{doc.metadata.get('chunk_id')}"
            )
    ...
    for result in query_results:
        ...
        if resource_type == "document":
            ...
            chunks_sorted = sorted(
                result, key=itemgetter("document", "chunk_id")
            )
            context += "\n\n".join(
                document["content"] for document in chunks_sorted
            )
            ...
            if context:
                context += "\n\n"
            context += (
                "Here are some examples that can help you answer the question:\n\n"
                "### EXAMPLES BEGIN ###\n\n"
            )
            chunks_sorted = sorted(
                result, key=itemgetter("document", "chunk_id")
