...
REDIRECT_URL = os.getenv(
    "OIDC_ISSUER", "https://www.freva.dkrz.de/api/freva-nextgen/"
)
...
REQUIRED_SCOPES = [
    s.strip()
    for s in os.getenv("MCP_REQUIRED_SCOPES", "openid profile").split(",")
    if s.strip()
]

disc = requests.get(
    REDIRECT_URL.rstrip("/") + "/.well-known/openid-configuration", timeout=10
).json()
