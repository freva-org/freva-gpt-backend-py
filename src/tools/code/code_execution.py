...
MCP_KERNELS = Gauge("mcp_kernels", "Number of active kernels in code MCP server")
...
from src.tools.code.helpers import (
    strip_ansi, start_kernel, shutdown_kernel,
)
...
def cleanup_mcp_session(sid: str) -> None:
    ...
    MCP_KERNELS.set(len(KERNEL_REGISTRY))
    if km is not None:
...
def execute_code(sid: str, code: str) -> dict:
