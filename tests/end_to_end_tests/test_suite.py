...
def set_thread_topic(thread_id, new_topic):
    return do_request_and_maybe_fail(
        f"/setthreadtopic?thread_id={thread_id}&topic={new_topic}"
    )
...
def fork_thread(source_thread_id, user_index):
    return do_request_and_maybe_fail(
        f"/editthread?source_thread_id={source_thread_id}&user_index={user_index}"
    )
...
def test_third_request():
    ...
    _response2 = generate_full_response(
...
def test_edit_input():
    ...
    # We want the capability to edit a past user input.
    ...
    # Fork the thread at index 1, which is the second time the user spoke, overwriting the content about the information that I am a student of the university of Hamburg, but keeping the information about my name.
    fork_info = fork_thread(response1.thread_id, user_index=1)
    new_thread_id = fork_info["new_thread_id"]
    history = fork_info["history"]
    print("History of the forked thread: ", history)
    ...
    # But it should match the new_thread_id that was returned by the fork_thread endpoint.
    assert response3.thread_id == new_thread_id, (
        "The thread_id of the edited response does not match the new_thread_id returned by the fork_thread endpoint!"
    )

    # And that new thread_id should also be stored in the database, so we can test that.
    # We know partially what it should contain.
...
# Note that because this test suite was copied from the Rust backend, some comments might be outdated.
