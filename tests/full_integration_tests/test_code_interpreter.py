...
def _execute_code_via_mcp(mcp_c, code: str) -> Dict[str:Any]:
    ...
    if (
        ...
        and "structuredContent" not in results.keys()
    ):
        raise RuntimeError(
            "MCP client returned unknown result from code-interpreter."
        )
...
def test_eval_exec(mcp_client_CI) -> None:
    ...
    assert (
        _exec_and_get_evaluated_value(mcp_client_CI, {"code": "min(10,15)"})
        == "10"
    )
    assert (
        _exec_and_get_printed_value(mcp_client_CI, {"code": "print(2.5*2)"})
        == "5.0\n"
    )
...
def test_hard_crash(mcp_client_CI) -> None:
    ...
    code = {
        "code": "print('Code interpreter functions normally after hard crash!')"
    }
    assert (
        _exec_and_get_printed_value(mcp_client_CI, code)
        == "Code interpreter functions normally after hard crash!\n"
    )
